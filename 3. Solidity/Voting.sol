...
contract Voting is Ownable {
    ...
    */
    ...
    function vote(uint _proposal) public {
        ...
        require(proposals[_proposal].sessionId == sessionIdNonce, "This do not belong to the current voting session."); // With this check in place we also reject blank vote
     ...
     * return Winning Proposal for a specific session
     */
    function closeVotingSession() public{
        require(currentSessionState == WorkflowStatus.VotesTallied , "Session still not tallied");
        sessions[sessionIdNonce].winnerDefined = true;
        setWinningProposalID(sessionIdNonce); 

        sessionIdNonce += 1; //We're ready to start a new voting session
    }
    
    /** 
     * get voters details after the session ends
     */
    function getVotersDetaile(address _address) public view returns(uint _votedProposal) {
